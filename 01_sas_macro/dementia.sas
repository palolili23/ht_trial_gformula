...
libname rose 'V:\Uitwissel\Paloma\HT trial\gformula\dementia\sbp_smoke';

PROC IMPORT OUT= rstrial
            DATAFILE= "V:\Uitwissel\Paloma\HT trial\Setup_R\01_setup\data\ht_trial_dementia.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
RUN;
...
tables dementia_plr dead_plr combined_plr cens; 
run;
...
covmeandata = covmean, 
savelib = rose,
graphfile= dementia.pdf
);


data rose.survdata_dem;
set work.Survdata;
run;

data rose.natcourse_dem;
set work.Natcourse;
run;

data rose.covmean_dem;
set work.Covmean;
run;

data rose.results_dem;
set work.Results;
run;
